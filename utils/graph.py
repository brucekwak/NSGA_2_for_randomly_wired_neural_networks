...
def make_random_graph(num_graph, stage_pool_path_list):
    ...
    for path_i in stage_pool_path_list:
        ...
        if check_file_num < num_graph:
            ...
            while check_file_num < num_graph:
                Nodes = random.randint(20, 40)  # => [Nodes 값 수정 시 주의] WS의 K값은 Nodes보다 작아야함.
                
                if check_file_num < check_BR:
                    # BR
                    # M개의 노드를 만든 후, 노드를 1개씩 추가시키며 노드 개수가 총 N개가 될 때까지 반복함
                    graph_model = 'BA'
                    M = random.randint(5, Nodes-10)
                    ...
                    # ER
                    graph_model = 'ER'
                    P = round(random.uniform(0.3, 0.9), 2)   # 소수 둘째자리까지만 나오도록 반올림 => 결과 e.g. 0.75
                    ...
                    # WS
                    graph_model = 'WS'
                    K = random.randint(5, Nodes-10)  # [min, max] // WS에서는 K nearest. 따라서, (5, 10~20) random 선택 하도록
                    P = round(random.uniform(0.3, 0.9), 2)   # 소수 둘째자리까지만 나오도록 반올림 => 결과 e.g. 0.75
...
#     - 일단 WS로만 만듦
# _2020.02.14
# def make_random_graph_v3(num_graph, stage_pool_path_list):
#     # 각 stage 당 128개 그래프 생성 (아래 종류별 각각 43개, 43개, 42개 생성)
#     check_BR = 43   # 0 ~ 42
#     check_ER = check_BR + 43   # 86  <= 43 ~ 85
#     check_WS = check_ER + 42   # 128  <= 86 ~ 127
...
#             print("Start to make random graph pool...")
#             while check_file_num < num_graph:
#                 Nodes = random.randint(10, 30)  # => [Nodes 값 수정 시 주의] 아래 K값은 Nodes보다 작아야함.  ## 기존에는 (20, 40)            
                
#                 if check_file_num < check_BR:
#                     # BR 만들기
#                     graph_model = 'BA'
#                     M = random.randint(5, Nodes-2)  # M개의 노드를 만든 후, 노드를 1개씩 추가시키며 노드 개수가 총 N개가 될 때까지 반복함
#                      # randint(5, 10) ~ randint(5, 25)
...
#                     # WS 만들기
#                     graph_model = 'WS'
#                     K = random.randint(5, Nodes-2)  # [min, max] // WS에서는 K nearest. 따라서, (5, 10~20) random 선택 하도록
#                     P = round(random.uniform(0.5, 0.9), 2)   # 소수 둘째자리까지만 나오도록 반올림 => 결과 e.g. 0.75
